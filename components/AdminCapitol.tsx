...
const AdminCapitol: React.FC = () => {
  ...
  if (!stats) return <div className="p-12 text-theme-secondary">Loading War Room...</div>;
  ...
  return (
    <div className="min-h-screen bg-theme-background text-theme-primary p-8 pb-32">
      ...
      <div className="flex justify-between items-center mb-12 border-b border-theme-border pb-8">
        <div>
          <h1 className="text-3xl font-black tracking-tighter uppercase text-theme-primary">The Capitol <span className="text-indigo-500 italic">Command</span></h1>
          <p className="text-xs font-bold text-theme-tertiary tracking-[0.3em] uppercase mt-1">Hindustan Mattress Co. Global View</p>
        </div>
        <div className="flex gap-4">
          <button className="bg-theme-card px-6 py-3 rounded-xl text-xs font-bold text-theme-secondary hover:bg-indigo-600 hover:text-white transition-all">SYSTEM HEALTH: OPTIMAL</button>
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-12">
        <div className="bg-theme-card border border-theme-border p-8 rounded-3xl">
          <span className="text-[10px] font-bold text-theme-tertiary uppercase tracking-widest block mb-4">Total Revenue Today</span>
          <div className="text-4xl font-black text-theme-primary">₹{(stats.revenue_today/1000).toFixed(1)}K</div>
          <div className="text-xs text-emerald-500 font-bold mt-2">+14% vs Yesterday</div>
        </div>
        <div className="bg-theme-card border border-theme-border p-8 rounded-3xl">
          <span className="text-[10px] font-bold text-theme-tertiary uppercase tracking-widest block mb-4">Pending Production</span>
          <div className="text-4xl font-black text-theme-primary">{stats.pipeline[ProductionStage.NEW] + stats.pipeline[ProductionStage.CUTTING] + stats.pipeline[ProductionStage.STITCHING]}</div>
        ...
        <div className="col-span-2 bg-theme-card border border-theme-border p-8 rounded-3xl overflow-hidden relative">
          <div className="flex justify-between items-end h-full">
            <div className="relative z-10">
              <span className="text-[10px] font-bold text-theme-tertiary uppercase tracking-widest block mb-4">Demand Hotspots</span>
              <div className="space-y-4">
                {stats.hotspots.map(h => (
                  <div key={h.city} className="flex items-center gap-4">
                    <div className="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
                    <span className="text-theme-primary font-bold text-sm">{h.city}</span>
                    <span className="text-[10px] text-theme-tertiary uppercase">{h.count} Orders</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div className="space-y-6">
            {Object.entries(stats.pipeline).map(([stage, count]) => (
              <div key={stage} className="flex items-center justify-between group">
                <span className="text-[10px] font-bold text-theme-tertiary uppercase tracking-widest group-hover:text-indigo-400 transition-colors">{stage}</span>
                ...
                <span className="text-sm font-black text-theme-primary">{count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-indigo-900/5 border border-indigo-500/10 rounded-3xl p-10 flex flex-col justify-center text-center">
           ...
           <h3 className="text-xl font-bold text-theme-primary mb-2 uppercase tracking-tighter">Tax & Compliance Hub</h3>
           <p className="text-xs text-theme-tertiary mb-8 max-w-xs mx-auto">Export GSTR-1, GSTR-3B, and dealer commission ledger for the current fiscal period.</p>
           <button 
             onClick={async () => {
      ...
      {isArmoryOpen && (
        <div className="fixed inset-0 bg-theme-background/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
           <div className="bg-theme-card border border-theme-border w-full max-w-lg p-12 rounded-[3rem] shadow-theme-2xl relative">
              <button onClick={() => setIsArmoryOpen(false)} className="absolute top-8 right-8 text-theme-tertiary hover:text-theme-primary transition-colors">
              ...
              <div className="text-center mb-10">
                 ...
                 <h2 className="text-2xl font-black text-theme-primary uppercase tracking-tighter italic">Armory <span className="text-brand-amber">Uploader</span></h2>
                 <p className="text-xs text-theme-tertiary mt-2 italic">Batch update 5,000+ variants in sub-2 seconds.</p>
              </div>
              ...
              {!uploadStatus ? (
                ...
                >
                   <span className="text-xs font-bold text-theme-tertiary group-hover:text-brand-amber">DROP MASTER_PRICING.CSV HERE</span>
                </div>
              ) : (
                <div className="text-center py-12 space-y-4">
                   <div className="text-4xl font-black text-brand-amber animate-pulse">SUCCESS</div>
                   <p className="text-sm font-bold text-theme-primary">Processed {uploadStatus.processed} variants in {uploadStatus.time_ms}ms</p>
                </div>
              )}
           </div>
        </div>
