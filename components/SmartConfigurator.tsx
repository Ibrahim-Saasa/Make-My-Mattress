  ...
  return (
    <div className="flex flex-col h-screen bg-theme-background">
      ...
      <div className="h-1/2 bg-theme-card relative flex items-center justify-center overflow-hidden border-b border-theme-border">
        <button onClick={onBack} className="absolute top-6 left-6 z-20 text-theme-secondary hover:text-indigo-700 transition-colors">
        ...
        <div className="relative w-full max-w-md transition-all duration-700 ease-out" style={{ perspective: '1200px' }}>
          <div 
            ...
            style={{ 
              ...
              background: 'linear-gradient(135deg, var(--color-card-background) 0%, var(--color-card-background-hover) 100%)'
            }}
          >
        ...
        <div className="absolute bottom-8 left-8">
           <div className="flex items-center gap-2 mb-1">
             <span className="text-4xl font-black text-theme-primary tracking-tighter">{params.length}" × {params.breadth}"</span>
      ...
      <div className="h-1/2 overflow-y-auto p-8 space-y-10 pb-40">
        ...
        <section className="pt-4"> {/* Added pt-4 here */}
        ...
        <section className="space-y-6">
          <div>
            <label className="text-[10px] font-bold text-theme-secondary uppercase tracking-widest mb-3 block">Frame Length</label>
              ...
              {LENGTHS.map(l => (
                <button 
                  ...
                  className={`px-6 py-3 rounded-2xl text-sm font-bold transition-all border hover:scale-[1.05] ${
                    params.length === l ? 'bg-indigo-600 text-white border-indigo-600 shadow-theme-medium' : 'bg-theme-input text-theme-secondary border-theme-border shadow-theme-light'
                  }`}
                >
                  {l}"
                </button>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <label className="text-[10px] font-bold text-theme-secondary uppercase tracking-widest block">Frame Breadth</label>
              <button 
            ...
            <div className="flex flex-wrap gap-2">
              {BREADTHS.map(b => (
                <button 
                  ...
                  className={`px-6 py-3 rounded-2xl text-sm font-bold transition-all border hover:scale-[1.05] ${
                    params.breadth === b ? 'bg-indigo-600 text-white border-indigo-600 shadow-theme-medium' : 'bg-theme-input text-theme-secondary border-theme-border shadow-theme-light'
                  }`}
                >
                  {b}"
                </button>
      ...
      <div className="fixed bottom-0 w-full bg-theme-card/95 backdrop-blur-xl border-t border-theme-border p-6 z-30">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-theme-secondary uppercase block mb-1">Estimated Total</span>
      ...
      {isCustomModalOpen && (
        <div className="fixed inset-0 bg-brand-navy/70 backdrop-blur-md z-50 flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-theme-card rounded-[3rem] w-full max-w-sm p-12 shadow-theme-2xl shadow-indigo-500/20">
            <h3 className="text-2xl font-black text-theme-primary mb-2">Custom Build</h3>
            <p className="text-sm text-theme-secondary mb-8">Pricing is calculated dynamically based on square-inch surface area.</p>
            <div className="space-y-4 mb-10">
              <div>
                <label className="text-[10px] font-bold text-theme-secondary uppercase mb-2 block">Length (Inch)</label>
                <input 
                  type="number"
                  value={customInput.l}
              ...
              <div>
                <label className="text-[10px] font-bold text-theme-secondary uppercase mb-2 block">Breadth (Inch)</label>
                <input 
                  type="number"
                  value={customInput.b}
            ...
            <div className="flex gap-4">
               <button onClick={() => setCustomModalOpen(false)} className="flex-1 py-4 font-bold text-theme-secondary hover:text-theme-primary">Cancel</button>
