...
export function normalizeScores(scores: TagWeights): { tag: string; score: number }[] {
...
export function mapTagToRecommendation(tag: string) {
  const mapping: Record<string, { type: string; models: string[] }> = {
    firm: { type: "Firm Hybrid / Innerspring", models: ["Firm Classic", "Innerspring Pro"] },
    medium: { type: "Medium-Firm Hybrid", models: ["Allrounder Medium", "Pocket Medium"] },
    soft: { type: "Memory Foam Plush", models: ["Cloud Plush", "Pillow Top"] },
    pressure_relief: { type: "Zoned Memory Foam", models: ["Zoned Relief", "Contour Plus"] },
    cooling: { type: "Latex / Cooling Hybrid", models: ["Cool Latex", "Hybrid Breeze"] },
    motion_isolation: { type: "Memory Foam / Isolated", models: ["Silent Sleep", "MotionGuard"] },
    support: { type: "Supportive Hybrid", models: ["SpineAlign", "SupportPro"] },
    durability: { type: "Durable Build", models: ["Endurance", "Lifetime Plus"] },
    value: { type: "Value Pick", models: ["Budget Friendly", "Value Comfort"] },
    premium: { type: "Luxury Line", models: ["Opulence", "Signature"] },
  };

  return mapping[tag] || { type: "Balanced / Hybrid", models: ["Standard Hybrid"] };
}
